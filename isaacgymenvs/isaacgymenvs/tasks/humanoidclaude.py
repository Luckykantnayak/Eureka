...
class HumanoidClaude(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.potentials, self.prev_potentials, self.up_vec, self.actions, self.dt)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, actions: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Extract position and velocity from root_states
    torso_position = root_states[:, 0:3]
    velocity = root_states[:, 7:10]
    
    # Forward velocity reward (primary objective - run fast)
    forward_velocity = velocity[:, 0]  # x-direction velocity
    velocity_temp: float = 1.0
    velocity_reward = torch.exp(velocity_temp * forward_velocity)
    
    # Progress reward (moving toward target)
    progress_reward = potentials - prev_potentials
    
    # Upright reward (stay upright while running)
    up_reward = up_vec[:, 2]  # z-component of up vector
    upright_temp: float = 5.0
    upright_reward = torch.exp(upright_temp * (up_reward - 0.9))
    
    # Survival reward (keep torso above ground)
    torso_height = torso_position[:, 2]
    survival_temp: float = 10.0
    survival_reward = torch.exp(survival_temp * (torso_height - 0.8))
    
    # Energy efficiency penalty (discourage excessive actions)
    action_penalty = -torch.sum(actions * actions, dim=-1)
    energy_temp: float = 0.01
    energy_reward = torch.exp(energy_temp * action_penalty)
    
    # Lateral stability (minimize sideways movement)
    lateral_velocity = torch.abs(velocity[:, 1])  # y-direction velocity
    stability_temp: float = 2.0
    stability_reward = torch.exp(-stability_temp * lateral_velocity)
    
    # Combine rewards with appropriate weights
    total_reward = (2.0 * velocity_reward + 
                   1.0 * progress_reward + 
                   0.5 * upright_reward + 
                   0.5 * survival_reward + 
                   0.1 * energy_reward + 
